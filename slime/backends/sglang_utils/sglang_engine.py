...
def _wait_server_healthy(base_url, api_key, is_process_alive):
    ...
    request_timeout = float(os.environ.get("SLIME_SGLANG_HEALTH_TIMEOUT", "5"))
    max_wait = float(os.environ.get("SLIME_SGLANG_HEALTH_MAX_WAIT", "600"))
    deadline = time.monotonic() + max_wait
    next_log_time = time.monotonic() + 30
    ...
    with requests.Session() as session:
        ...
        while True:
            ...
            if not is_process_alive():
                raise Exception(f"Server process terminated unexpectedly while waiting for {base_url}/health_generate.")

            now = time.monotonic()
            if now >= deadline:
                raise TimeoutError(
                    f"Timed out after {max_wait:.1f}s waiting for {base_url}/health_generate "
                    f"(per-request timeout={request_timeout:.1f}s, last_error={last_error})."
                )
            if now >= next_log_time:
                logger.info(
                    "Waiting for SGLang server health at %s/health_generate; last_error=%s",
                    base_url,
                    last_error,
                )
                next_log_time = now + 30
...
class SGLangEngine(RayActor):
    ...
    def _init_external(self, expect_server_args, external_engine_need_check_fields):
        ...
        def _get_actual_server_args():
            response = _request_without_proxy("GET", f"http://{self.server_host}:{self.server_port}/get_server_info")
    ...
    def _init_normal(self, server_args_dict):
        ...
        if self.node_rank == 0 and self.router_ip and self.router_port:
            if parse(sglang_router.__version__) <= parse("0.2.1"):
                ...
                response = _request_without_proxy(
                    "POST",
    ...
    def _make_request(self, endpoint: str, payload: dict | None = None):
        ...
        response = _request_without_proxy("POST", url, json=payload or {})
        try:
            response.raise_for_status()
    ...
    def flush_cache(self):
        ...
        for _ in range(60):
            try:
                response = _request_without_proxy("GET", f"http://{self.server_host}:{self.server_port}/flush_cache")
                if response.status_code == 200:
                    break
    ...
    def shutdown(self):
        ...
        if self.worker_type != "encoder" and self.node_rank == 0:
            ...
            if parse(sglang_router.__version__) <= parse("0.2.1"):
                ...
                response = _request_without_proxy("DELETE", f"http://{self.router_ip}:{self.router_port}/workers/{worker_url}")
            else:
                try:
                    all_workers = _request_without_proxy("GET", f"http://{self.router_ip}:{self.router_port}/workers").json()["workers"]
    ...
    def get_weight_version(self):
        ...
        response = _request_without_proxy("GET", url)
        response.raise_for_status()
    ...
    def pause_generation(self):
        response = _request_without_proxy("POST", f"http://{self.server_host}:{self.server_port}/pause_generation", json={})
        response.raise_for_status()
        return response

    def continue_generation(self):
        response = _request_without_proxy("POST", f"http://{self.server_host}:{self.server_port}/continue_generation", json={})
        response.raise_for_status()
        return response
    ...
    def stop_profile(self):
        response = _request_without_proxy("POST", f"http://{self.server_host}:{self.server_port}/stop_profile", json={})
        response.raise_for_status()
        return response
