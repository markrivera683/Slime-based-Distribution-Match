#!/usr/bin/env bash
# Smoke wrapper for OPD-CF-L1OO training.
#
# This uses OPD teacher logprobs over student on-policy rollouts as the teacher
# signal, and disables the old teacher-completion target path.
set -euo pipefail
...
export OPD_CREDIT_ASSIGNMENT="${OPD_CREDIT_ASSIGNMENT:-cf_l1oo}"
...
export G2_OPD_MODE="${G2_OPD_MODE:-opd_cf_l1oo_sglang}"
...
export RUN_NAME="${RUN_NAME:-opd_cf_l1oo_smoke_$(date +%m%d_%H%M%S)}"
