...
- `artifacts/g1_runtime_megatron_ref_mask_applied_full_group_dump.pt`: full-group Megatron/ref dump produced with `--g1-megatron-ref-forward-mode openrlhf_exact --g1-megatron-ref-apply-dense-attention-mask`.
...
"${PYTHON_BIN}" scripts/dump_openrlhf_g1_from_megatron_dump.py \
  ...
  --out artifacts/g1_runtime_openrlhf_critic_mask_applied_full_group_dump.pt \
...
"${PYTHON_BIN}" scripts/compare_g1_runtime_parity.py \
  ...
  --out g1_runtime_parity_report_mask_applied_full_group.md
```

## Expected Checks

In `g1_runtime_parity_report_mask_applied_full_group.md`, check:
