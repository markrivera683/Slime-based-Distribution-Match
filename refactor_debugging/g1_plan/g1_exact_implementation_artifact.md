...
本轮把 G1 迁移从“reward/RLOO/token advantage 数学已对齐”推进到了“慢 HF/OpenRLHF embedding path 可以接入 slime rollout 小闭环”的阶段。
...
```text
SGLang rollout
  -> custom_generate: generate_with_g1_embeddings
  -> fixed response length check: 376
  -> slow OpenRLHF/HF critic embedding
  -> metadata["g1_gen_embedding"] / metadata["g1_gt_embedding"]
  -> group_rm custom_rm
  -> metadata["g1_token_advantages"]
  -> RolloutManager train_data
  -> actor.py moves tensor to GPU
...
- real smoke run with model and rollout
- full `prompt -> critic hidden -> groom` runtime golden parity
- Megatron / fast embedding producer
...
--use-whitening
--rollout-max-response-len 376
--g1-critic-model-path <HF critic path>
