#!/usr/bin/env bash
# Full standalone 1-node 8-GPU OPD-CF-L1OO Slime/Megatron run for Qwen3.5-2B.
#
# Default layout:
#   teacher Qwen3.5-27B SGLang scorer: GPUs 0,1,2,3
...
TEACHER_LOG_DIR="${TEACHER_LOG_DIR:-/tmp/slime_opd_cf_l1oo_1node8}"
...
TENSOR_MODEL_PARALLEL_SIZE_WAS_SET="${TENSOR_MODEL_PARALLEL_SIZE+x}"
ROLLOUT_NUM_GPUS_PER_ENGINE_WAS_SET="${ROLLOUT_NUM_GPUS_PER_ENGINE+x}"
ACTOR_NUM_GPUS_PER_NODE_WAS_SET="${ACTOR_NUM_GPUS_PER_NODE+x}"
CRITIC_NUM_GPUS_PER_NODE_WAS_SET="${CRITIC_NUM_GPUS_PER_NODE+x}"
ROLLOUT_NUM_GPUS_WAS_SET="${ROLLOUT_NUM_GPUS+x}"
...
# ---------------------------------------------------------------------------
G2_OPD_MODE="${G2_OPD_MODE:-opd_cf_l1oo_sglang_1node8}"
...
EFFOPD_VALIDATION_MODE="${EFFOPD_VALIDATION_MODE:-combined_gate}"
...
RUN_NAME="${RUN_NAME:-g2_opd_cf_l1oo_qwen35_2b_1node8_$(date +%m%d_%H%M%S)}"
...
# must be in NO_PROXY before teacher preflight and Ray worker startup.
...
if [[ -n "${TEACHER_HOST:-}" ]]; then
  SLIME_NO_PROXY_EXTRA="${SLIME_NO_PROXY_EXTRA},${TEACHER_HOST}"
fi
if [[ -n "${TEACHER_BIND_HOST:-}" ]]; then
  SLIME_NO_PROXY_EXTRA="${SLIME_NO_PROXY_EXTRA},${TEACHER_BIND_HOST}"
fi
if [[ -n "${NO_PROXY:-}" ]]; then
...
launch_teacher_node() {
  ...
  local teacher_bind_host="${TEACHER_BIND_HOST:-127.0.0.1}"
  local teacher_cuda_visible_devices="${TEACHER_CUDA_VISIBLE_DEVICES:-0,1,2,3}"
  local teacher_tp_size="${TEACHER_TP_SIZE:-2}"
  local teacher_dp_size="${TEACHER_DP_SIZE:-2}"
  local teacher_context_length="${TEACHER_CONTEXT_LENGTH:-2048}"
  ...
  echo "[1node8-opd-cf-l1oo-teacher] CUDA_VISIBLE_DEVICES=${teacher_cuda_visible_devices}"
  echo "[1node8-opd-cf-l1oo-teacher] model=${teacher_model_path} served=${teacher_served_model_name}"
  echo "[1node8-opd-cf-l1oo-teacher] bind=${teacher_bind_host}:${TEACHER_PORT} TP=${teacher_tp_size} DP=${teacher_dp_size} context=${teacher_context_length} mem=${teacher_mem_fraction_static}"
  printf "[1node8-opd-cf-l1oo-teacher] command:"
  printf " %q" "${cmd[@]}"
  printf "\n"
...
cleanup_teacher() {
  if [[ -n "${teacher_pid}" && "${KEEP_TEACHER_ALIVE}" != "true" ]]; then
    echo "[1node8-opd-cf-l1oo] stopping teacher pid=${teacher_pid}"
...
start_local_teacher_background() {
  ...
  echo "[1node8-opd-cf-l1oo] starting teacher on GPUs ${TEACHER_CUDA_VISIBLE_DEVICES}; log=${TEACHER_LOG}"
  launch_teacher_node >"${TEACHER_LOG}" 2>&1 &
  teacher_pid="$!"
}

wait_for_opd_logprob_endpoint() {
  local url="$1"
  echo "[1node8-opd-cf-l1oo] waiting for OPD logprob endpoint: ${url}"
    ...
    try:
        ...
        if isinstance(token_logprobs, list) and len(token_logprobs) >= 2:
            ...
            if any(isinstance(value, (int, float)) for value in scored):
                print("[1node8-opd-cf-l1oo] OPD logprob endpoint is ready")
                raise SystemExit(0)
        last_error = "missing numeric meta_info.input_token_logprobs"
    except Exception as exc:
        last_error = f"{type(exc).__name__}: {exc}"
    print(f"[1node8-opd-cf-l1oo] teacher not ready yet: {last_error}", file=sys.stderr)
...
if [[ "${PRINT_MODE}" == "true" ]]; then
  echo "[1node8-opd-cf-l1oo] PRINT_ONLY/DRY_RUN_ONLY set; skipping teacher startup and endpoint wait"
elif [[ "${START_TEACHER}" == "true" ]]; then
  start_local_teacher_background
  wait_for_opd_logprob_endpoint "${OPD_TEACHER_RM_URL}"
else
  echo "[1node8-opd-cf-l1oo] START_TEACHER=false; using existing teacher at ${OPD_TEACHER_RM_URL}"
  wait_for_opd_logprob_endpoint "${OPD_TEACHER_RM_URL}"
fi
...
if [[ "${ENABLE_EFFOPD}" == "true" ]]; then
  COLOCATE="false"
  if [[ -z "${TENSOR_MODEL_PARALLEL_SIZE_WAS_SET}" ]]; then
    TENSOR_MODEL_PARALLEL_SIZE="1"
  fi
  if [[ -z "${ROLLOUT_NUM_GPUS_PER_ENGINE_WAS_SET}" ]]; then
    ROLLOUT_NUM_GPUS_PER_ENGINE="1"
  fi
  if [[ -z "${ACTOR_NUM_GPUS_PER_NODE_WAS_SET}" ]]; then
    ACTOR_NUM_GPUS_PER_NODE="1"
  fi
  if [[ -z "${CRITIC_NUM_GPUS_PER_NODE_WAS_SET}" ]]; then
...
if [[ "${COLOCATE}" == "false" ]]; then
  if [[ -z "${ROLLOUT_NUM_GPUS+x}" ]] || { [[ "${ENABLE_EFFOPD}" == "true" ]] && [[ -z "${ROLLOUT_NUM_GPUS_WAS_SET}" ]]; }; then
