#!/usr/bin/env bash
# Full standalone G2 cf_l1oo Slime/Megatron run for Qwen3.5-2B (OPD disabled).
#
# Deployment layouts:
...
#   bash exper_scripts/main_test/run_g2_cf_l1oo_qwen35_2b_2node.sh
# Or manually:
#   bash exper_scripts/main_test/start_g2_sglang_teacher_2node.sh   # teacher node
#   TEACHER_HOST=<ip> bash exper_scripts/main_test/run_g2_cf_l1oo_qwen35_2b_main_2node_student.sh
#
# Single-node / explicit API base:
#   TEACHER_API_BASE=http://127.0.0.1:30000 \
#   TEACHER_MODEL_NAME=qwen3.5-27b \
#   bash exper_scripts/main_test/run_g2_cf_l1oo_qwen35_2b_main.sh
set -euo pipefail
...
# ---------------------------------------------------------------------------
# 6. G2 counterfactual teacher (cf_l1oo only; OPD disabled)
# ---------------------------------------------------------------------------
G2_OPD_MODE="${G2_OPD_MODE:-cf_l1oo}"
...
TEACHER_SYSTEM_PROMPT_ID="${TEACHER_SYSTEM_PROMPT_ID:-g2-cf-l1oo-main-v1}"
...
RUN_NAME="${RUN_NAME:-g2_cf_l1oo_qwen35_2b_main_$(date +%m%d_%H%M%S)}"
...
RAY_TMPDIR="${RAY_TMPDIR:-/tmp/ray_g2_cf_l1oo_main_${USER:-user}}"
...
if [[ "${TEACHER_BACKEND}" != "remote" ]]; then
  echo "[ERROR] G2 cf_l1oo main supports TEACHER_BACKEND=remote only, got: ${TEACHER_BACKEND}" >&2
  exit 1
fi
...
if [[ -z "${TEACHER_API_BASE}" || -z "${TEACHER_MODEL_NAME}" ]]; then
  echo "[ERROR] TEACHER_API_BASE and TEACHER_MODEL_NAME must be set." >&2
  exit 1
fi
...
echo "[main-test] G2 cf_l1oo Slime/Megatron run (OPD disabled)"
...
echo "[main-test] teacher=${TEACHER_API_BASE} style=${TEACHER_API_STYLE} model=${TEACHER_MODEL_NAME}"
echo "[main-test] cf_l1oo lambda=${CF_TEACHER_LAMBDA} teacher_samples=${CF_TEACHER_N_SAMPLES} teacher_cache=${TEACHER_CACHE_ENABLE}"
