#!/usr/bin/env bash
# Student node for two-node G2+OPD (mandatory: Standard G2 cf_l1oo + Slime OPD).
#
# Reward / training recipe (same as G2 smoke / EBFT G2 rebase):
...
#   --use-opd --opd-type sglang for teacher logprobs (Slime native OPD path)
#   frozen critic: --critic-lr 0 --critic-lr-head 0
#
# Slime execution stack (same machinery as G1 main, different reward head):
#   train_async.py: Megatron actor+critic || SGLang student rollout
#   Megatron ref + g1_embedding for token advantages underlying cf_l1oo
#   SGLang rollout engines on 4 GPUs; remote 27B teacher on other node
#
# 8-GPU split (TP=2): actor 2 + critic 2 + SGLang rollout 4.
#
# Prerequisites:
#   Teacher node: bash exper_scripts/main_test/start_g2_sglang_teacher_2node.sh
#
# Example:
#   TEACHER_HOST=10.0.0.1 \
#   RAY_NODE_IP_ADDRESS=10.0.0.2 \
#   bash exper_scripts/main_test/run_g2_opd_qwen35_2b_main_2node_student.sh
set -euo pipefail
...
# --- Standard G2 cf_l1oo (required; wired in run_g2_opd_qwen35_2b_main.sh) ---
export G2_OPD_MODE="${G2_OPD_MODE:-cf_l1oo_opd_sglang}"
...
# --- Remote teacher node: cf_l1oo online completions + OPD logprobs ---
