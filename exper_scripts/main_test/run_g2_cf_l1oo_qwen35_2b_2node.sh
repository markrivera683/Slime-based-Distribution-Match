#!/usr/bin/env bash
# Orchestrate G2 cf_l1oo teacher (8 GPU) + student (8 GPU) in one entrypoint.
#
...
#   bash .../exper_scripts/main_test/run_g2_cf_l1oo_qwen35_2b_2node_dlc.sh
#
...
#
# Local 4+4 debug:
#   DEPLOY_MODE=local_split bash exper_scripts/main_test/run_g2_cf_l1oo_qwen35_2b_2node.sh
set -euo pipefail
...
STUDENT_SCRIPT="${STUDENT_SCRIPT:-${SCRIPT_DIR}/run_g2_cf_l1oo_qwen35_2b_main_2node_student.sh}"
...
TEACHER_LOG="${TEACHER_LOG:-${DLC_LOCAL_ROOT}/slime_logs/g2_cf_l1oo_teacher.log}"
TEACHER_PID_FILE="${TEACHER_PID_FILE:-${DLC_LOCAL_ROOT}/slime_logs/g2_cf_l1oo_teacher.pid}"
...
setup_dlc_runtime_env() {
  ...
  export RAY_TMPDIR="${RAY_TMPDIR:-${DLC_LOCAL_ROOT}/ray_slime_g2_cf_l1oo}"
...
setup_dlc_stable_run_name() {
  ...
  if [[ -n "${job_id}" && "${job_id}" != "$(hostname)" ]]; then
    export RUN_NAME="g2_cf_l1oo_${job_id}"
  fi
}
